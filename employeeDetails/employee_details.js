...
function findJavaScriptSpecialization(){
    const foundJavaScript = employees.find(employee => employee.specialization === 'JavaScript');

    document.getElementById('employeesDetails').innerHTML =`<p>${foundJavaScript.id}: ${foundJavaScript.name}: ${foundJavaScript.name} - ${foundJavaScript.department} - $${foundJavaScript.salary} - ${foundJavaScript.specialization}</p>`;
}